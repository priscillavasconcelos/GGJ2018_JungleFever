...
public class PlayerControl : MonoBehaviour
{
    ...
    public Text textoBananas;
    //public Image tutoPreto;
	...
	void Awake()
	{
        ...
        textoBananas.text = bananaCount.ToString() + "/3";
	}

    private void OnTriggerEnter2D(Collider2D collision)
    {
        if (collision.name == "TriggerSomeTexto")
        {
            //Destroy(collision.gameObject);
            GameMaganer.GM.CallFade();
        }
    ...
    {
        if (collision.transform.tag == "Cipo" && canSwing)
        {
            canSwing = false;
            swinging = true;
        ...
        {
            //porrada.Play();
            StartCoroutine(playSomeNovoLevel());
        }
    }
	...
	void FixedUpdate ()
	{
        ...
        else
        {
            ...
            GetComponent<Rigidbody2D>().AddForce(transform.right * (h * swingForce));
